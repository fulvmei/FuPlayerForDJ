...
public final class MediaSessionPlayer {
    ...
    public interface MediaLoadProvider {
        ...
        void onPrepare(Player player, MediaDescriptionCompat description);
    }
...
//                extras.setClassLoader(TimingOff.class.getClassLoader());
        ...
        private void prepare(MediaDescriptionCompat description) {
            MediaItem mediaItem = new MediaItem.Builder()
                    .setMediaId(mActiveQueueItem.getDescription().getMediaId())
                    .setUri(mActiveQueueItem.getDescription().getMediaUri())
                    .setTag(mActiveQueueItem)
                    .build();

            mPlayer.setMediaItem(mediaItem);
            mPlayer.prepare();
            if (mMediaLoadProvider != null) {
                mMediaLoadProvider.onPrepare(mPlayer, description);
            }
        }
        ...
        public void onRewind() {
            if (mPlayer.isCurrentMediaItemSeekable() && rewindMs > 0) {
        ...
        public void onFastForward() {
            if (mPlayer.isCurrentMediaItemSeekable() && fastForwardMs > 0) {
